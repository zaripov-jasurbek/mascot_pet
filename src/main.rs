...
const CATCH_RANGE_FRAC: f32 = 0.40; // дистанция ловли / ширину окна
const GRAVITY_FRAC: f32 = 2.5; // гравитация / высоту экрана (пикс/с²)
const CATCH_HAND_DX_FRAC: f32 = 0.15; // рука/захват левее центра окна (доля ширины)
...
const JUMP_LIFT: f32 = 0.15; // подъём модели по +Y во время прыжка (чтобы ноги не обрезались)
const SLOW_FALL_FACTOR: f32 = 0.35; // во сколько медленнее падение после поимки (hanging)
...
struct Config {
    ...
    catch_range: f32,
    gravity: f32,
    catch_hand_dx: f32, // сдвиг точки ловли/удержания влево от центра окна
}
...
impl Config {
    fn from_work_area() -> Self {
        ...
        Self {
            ...
            catch_hand_dx: win_w as f32 * CATCH_HAND_DX_FRAC,
        }
    }
    ...
    // Смещение до живота.
    fn belly_offset(&self) -> f32 {
        self.win_h as f32 - 0.5 * self.char_px
    }
    fn min_x(&self) -> f32 {
    ...
    // Y середины экрана — выше неё верх окна при прыжке не поднимается.
    fn half_y(&self) -> f32 {
        (self.work_top + self.work_h / 2) as f32
    }
}
...
impl State {
    ...
    // Разовые анимации (прыжок) — играем один раз, замирают на последнем кадре.
    fn one_shot(self) -> bool {
        matches!(self, State::JumpingUp)
    }
...
struct Mascot {
    ...
    fall_slow: bool,       // медленное падение (после поимки)
...
fn falling_system(
    ...
    // свободное падение (после поимки — медленнее)
    let g = cfg.gravity * if mascot.fall_slow { SLOW_FALL_FACTOR } else { 1.0 };
    mascot.vel_y += g * dt;
    mascot.pos_y += mascot.vel_y * dt;

    // если держим украденный курсор — тянем его за рукой/животом
    if mascot.fall_hold {
        let hx = (mascot.pos_x + cfg.win_w as f32 / 2.0 - cfg.catch_hand_dx) as i32;
        let hy = mascot.pos_y as i32 + cfg.belly_offset() as i32;
...
fn chaos_system(
    ...
    // точка руки/ловли — левее центра окна (jumping_up уводит влево)
    let hand_x = center_x - cfg.catch_hand_dx;
    ...
    match mascot.state {
        ...
        State::ChaseRun => {
            ...
            let hand_now = mascot.pos_x + cfg.win_w as f32 / 2.0 - cfg.catch_hand_dx;
            let near = (cx as f32 - hand_now).abs() <= cfg.catch_range;
            if near {
                // добежали — прыгаем к курсору (цель: рука к курсору, не выше середины)
                mascot.jump_to =
                    (cy as f32 - cfg.hand_top_offset()).clamp(cfg.half_y(), cfg.ground_y());
                mascot.vel_y = 0.0;
        ...
        State::JumpingUp => {
            ...
            if p >= 1.0 {
                ...
                if dx <= cfg.catch_range && dy <= cfg.catch_range {
                    ...
                    start_fall(&mut mascot, &mut commands, &lib, &cfg, now, "hanging", true, true);
                } else {
                    info!(
                        "jump ✗ промах: курсор=({},{}) рука=({:.0},{:.0}) dx={:.0} dy={:.0}",
                        cx, cy, hand_x, hand_y, dx, dy
                    );
                    start_fall(&mut mascot, &mut commands, &lib, &cfg, now, "on_falling", false, false);
                }
            }
        }
        _ => {}
...
// ВРЕМЕННО: видимая рамка по краю окна (для отладки границ прозрачного окна).
fn spawn_debug_border(mut commands: Commands) {
    commands.spawn((
        Node {
        ...
        BorderColor::all(Color::srgba(1.0, 0.0, 0.0, 0.8)),
        // не перехватывать клики — иначе нельзя схватить модель мышью
        Pickable::IGNORE,
    ));
}
...
// Запустить падение: выбрать анимацию в воздухе, скорость (slow), держать ли курсор.
...
fn start_fall(
    ...
    mascot.fall_slow = slow;
    mascot.fall_hold = hold;
    mascot.vel_y = 0.0;
    let g = cfg.gravity * if slow { SLOW_FALL_FACTOR } else { 1.0 };
    let distance = (cfg.ground_y() - mascot.pos_y).max(0.0);
    let total = (2.0 * distance / g).sqrt();
    ...
    // если в воздухе нужна другая анимация (hanging при поимке) — переопределяем
    if air_anim != "on_falling" {
        play(commands, lib, air_anim, Duration::ZERO, RepeatAnimation::Forever);
    }
}
...
// Дыхание (покачивание по Y) + подъём модели во время прыжка, чтобы при
// смещении корня анимацией ступни не уходили за нижний край.
fn breathing_system(
    time: Res<Time>,
    mascot: Res<Mascot>,
    mut query: Query<&mut Transform, With<VrmRoot>>,
    ...
    let t = time.elapsed_secs();
    let lift = if mascot.state == State::JumpingUp {
        JUMP_LIFT
    } else {
        0.0
    };
    for mut tf in &mut query {
        tf.translation.y = (t * 1.7).sin() * 0.005 + lift;
    }
}
